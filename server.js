...
const APP_VERSION = "2026-06-08-ui-netease-immersive-v200";
const envList = (name) => String(process.env[name] || "")
  .split(",")
  .map((item) => item.trim())
  .filter(Boolean);
const NETEASE_PERSONAL_RADAR_ID = String(process.env.NETEASE_PERSONAL_RADAR_ID || "").trim();
const NETEASE_LIBRARY_PLAYLIST_ID = String(process.env.NETEASE_LIBRARY_PLAYLIST_ID || "").trim();
const NETEASE_IMPORTED_PLAYLIST_IDS = envList("NETEASE_IMPORTED_PLAYLIST_IDS");
const NETEASE_FAVORITE_PLAYLIST_IDS = envList("NETEASE_FAVORITE_PLAYLIST_IDS");
const NETEASE_PLAYLIST_NAMES = {};
...
function wantsPendingTitlePlayback(prompt) {
  const text = normalizeText(prompt);
  return /直接播放|直接播|播放就行|播就行|就这个|就它|就这首|不用确认|不用问|不用管版本|随便哪个版本|原声版就行|默认版本/.test(text);
}

function extractRequestedTitle(prompt) {
  const text = String(prompt || "").trim();
  const patterns = [
    /(?:我要听|我想听|想听|播放|放一下|放一首|来一首|给我放|给我播)\s*《([^》]{1,120})》/i,
    /(?:我要听|我想听|想听|播放|放一下|放一首|来一首|给我放|给我播)\s+(.{1,120})$/i,
    /(?:我要听|我想听|想听|播放|放一下|放一首|来一首|给我放|给我播)(.{1,120})$/i
  ];
...
async function getSongUrl(songId) {
  ...
  songUrlCache.set(cacheKey, { expiresAt: Date.now() + 8 * 60 * 1000, promise });
  return promise;
}
...
function isDjVersionTrack(track = {}) {
  ...
  return /[（(【\[]\s*dj[^）)\]】]*(?:版|version|remix|mix|串烧|舞曲|车载|慢摇|抖音|快手|弹鼓|加速|变速)?[^）)\]】]*[）)\]】]/i.test(text)
    || /(?:^|[\s·\-_/（(【\[])(?:dj|d\.j\.?)\s*(?:版|version|remix|mix|串烧|舞曲|车载|慢摇|抖音|快手|弹鼓|加速|变速|club|bootleg|extended)/i.test(text)
    || /(?:版|version|remix|mix|串烧|舞曲|车载|慢摇|抖音|快手|弹鼓|加速|变速|club|bootleg|extended)\s*(?:dj|d\.j\.?)(?:$|[\s·\-_/）)】\]])/i.test(text)
    || /(?:车载|抖音|快手|慢摇|弹鼓|土嗨|夜店|串烧).{0,8}(?:dj|d\.j\.?)|(?:dj|d\.j\.?).{0,8}(?:车载|抖音|快手|慢摇|弹鼓|土嗨|夜店|串烧)/i.test(text)
    || /dj版|djversion|djremix|djmix|dj舞曲|dj车载|dj串烧|dj慢摇|dj弹鼓|抖音dj|快手dj|车载dj|慢摇dj|夜店dj|土嗨dj/i.test(compact);
}
...
function isSpeedAlteredTrack(track = {}) {
  ...
  return /(?:^|[\s\-_/·.()[\]（）【】])(?:0[,.][5-9]|1[,.][1-9]|2[,.]0)\s*(?:x|倍速|速|版)?(?:$|[\s\-_/·.()[\]（）【】])/i.test(text)
...
function neteaseSongTags(song = {}) {
  ...
  if (Number.isFinite(pop) && pop >= 50) tags.push(`超${Math.min(99, Math.round(pop))}%人播放`);
...
async function getNeteaseProfile(base) {
  ...
  if (!uid) throw new Error("网易云未登录，无法同步账号歌单");
  return { uid, profile };
}
...
async function readNeteaseSourceCards() {
  const base = (process.env.NETEASE_API_BASE || "http://localhost:4000").replace(/\/$/, "");
  const cards = [];
  const pushCard = (id, name, item) => {
    cards.push({
      id,
      name,
  ...
  try {
    pushCard("daily", "每日推荐", await readNeteaseDynamicSource("daily"));
  } catch {
    pushCard("daily", "每日推荐", null);
  }
  try {
    pushCard("personal_fm", "私人雷达", await readNeteaseDynamicSource("personal_fm"));
  } catch {
    pushCard("personal_fm", "私人雷达", null);
  }
  for (const playlistId of NETEASE_IMPORTED_PLAYLIST_IDS) {
    const fallbackName = NETEASE_PLAYLIST_NAMES[playlistId] || `Playlist ${playlistId}`;
    try {
      const item = await readNeteasePlaylistTracks(base, { id: playlistId, name: fallbackName });
      item.source.name = NETEASE_PLAYLIST_NAMES[playlistId] || item.source.name || fallbackName;
      pushCard(`playlist-${playlistId}`, item.source.name, item);
    } catch {
      pushCard(`playlist-${playlistId}`, fallbackName, null);
    }
  }
  return { cards };
}
...
function fallbackHostLine({ track, nextTrack, weather, plan }) {
  ...
  const lines = [
    `${track.artist} 的 ${track.title} 可以从声音的“距离”听起：人声没有贴得太满，伴奏也没有急着抢镜，留出来的那一点空白正好让情绪自己浮上来。`,
    `把 ${track.title} 放回 ${album} 的语境里，它不像一首急着给结论的歌。${track.artist} 更像是在慢慢摆出一组颜色，等旋律走到中段，轮廓才清楚。`,
    `这首歌的入口不在副歌，而在前面那些细小的铺垫。${track.artist} 把节奏、气口和尾音收得很稳，听的时候可以让注意力先落在这些边缘。`,
    `${track.title} 有一种不把情绪端到台前的处理。它的好听不靠一下子推高，而是靠 ${track.artist} 把声音层次一点点打开。`,
    `如果把 ${track.title} 当作一段短片，镜头应该不是大特写，而是慢慢后退的长镜头。${track.artist} 留下的空间，比歌词本身更会说话。`,
    `${album} 里的这首 ${track.title}，最耐听的是它的重心。旋律往前走，但情绪没有被推着跑，像在一个很窄的光里缓慢移动。`,
    `现在听 ${track.title}，可以先不急着判断它属于哪种风格。${track.artist} 的处理更像把几种情绪叠在一起，让你自己在里面找到入口。`,
    `${track.title} 的质感比较适合小声一点听。音量降下来以后，${track.artist} 的声线、和声的阴影、伴奏的颗粒感会更明显。`,
    `这首不是靠“惊艳一下”成立的歌。${track.artist} 在 ${track.title} 里给的是一种持续的牵引，越往后越能听见它为什么要这样铺。`,
    `${track.title} 让我更想听它的结构，而不是只听旋律。${track.artist} 把段落之间的转折藏得不重，但每一次变化都在悄悄改空气。`,
    `从 ${album} 里抽出 ${track.title}，会发现它的情绪不是直线。它有一点回望的味道，但 ${track.artist} 没让它变成纯粹的怀旧。`,
    `${track.artist} 在这首里没有把表达做得很满，这反而让 ${track.title} 更像一封没写完的信。剩下的部分，要靠听的人自己补上。`,
    `${track.title} 的节拍像是给画面留了呼吸。它不催促，也不躲开情绪，只是让 ${track.artist} 的声音在一个合适的位置停住。`,
    `这首歌可以听它怎么处理“轻”。不是单薄，而是把不必要的重量拿掉，让 ${track.title} 在 ${album} 里显得更干净。`,
    `${track.artist} 的 ${track.title} 有一点适合独处的气质。不是把孤独说出来，而是让编曲和声线之间的距离替它说。`,
    `我会把 ${track.title} 的第一层理解放在音色上。${track.artist} 没有急着制造戏剧性，真正的起伏藏在声音纹理里。`,
    `${album} 给 ${track.title} 的背景很重要：它让这首歌听起来不是孤立的一段旋律，而像一页从旧本子里翻出来的记录。`,
    `现在这首 ${track.title}，适合听它怎么收尾。很多歌的性格不在高潮，而在最后几句怎么放手，${track.artist} 这里处理得很有分寸。`
  ];
...
function sanitizeHostLine(line, track) {
  ...
  const cleaned = String(line || "")
    ...
    .filter((sentence) => sentence && !/(下一首|下首|后面接|后面会|接下来|顺着|转场|会接|先接|播完|下一段|今天还有|日程节点|当前位置|天气)/.test(sentence))
    .join("");
  if (cleaned.length < 18 || /先不急|旋律把场景铺开|场景铺开|情绪落稳/.test(cleaned)) return fallback;
...
function wantsPlaybackAction(prompt) {
  return /听|播|播放|放|推荐|接下来|下一首|后面|之后|排|换成|切到|play|queue|next|after\s+this|after\s+this\s+song|put\s+on|listen\s+to|recommend|some\s+/i.test(normalizeText(prompt));
}
...
function extractStyleLabel(prompt) {
  ...
  if (styles.love) labels.push(/苦情|失恋|伤感|emo/i.test(prompt) ? "失恋 伤感 emo 慢歌 爱而不得 ballad" : "爱情 甜歌 浪漫 love ballad");
  ...
  return [...new Set(labels)].join(" ") || cleanQuery(prompt);
}

function semanticStyleQuery(prompt, style = "") {
  const raw = `${prompt || ""} ${style || ""}`;
  const normalized = normalizeText(raw);
  const parts = [];
  const add = (...items) => parts.push(...items.filter(Boolean));
  if (/英文|英语|欧美|english|western/i.test(normalized)) add("英文", "english");
  if (/华语|国语|中文|内地|港台|mandopop|c-?pop/i.test(normalized)) add("华语", "国语", "mandopop");
  if (/日语|日文|日系|日本|jpop|j-pop|anime/i.test(normalized)) add("日语", "j-pop", "anime");
  if (/r&b|rnb|rb|soul|布鲁斯|节奏布鲁斯/i.test(normalized)) add("r&b", "rnb", "soul", "neo soul", "rhythm blues");
  if (/苦情|失恋|伤感|emo|心碎|难过|爱而不得|虐/i.test(normalized)) add("失恋", "伤感", "emo", "慢歌", "爱而不得", "heartbreak", "sad", "ballad");
  else if (/情歌|爱情|恋爱|甜歌|浪漫|love/i.test(normalized)) add("爱情", "浪漫", "甜歌", "love", "heart", "ballad");
  if (/夜晚|深夜|睡前|安静|慢歌|慢节奏|舒缓/i.test(normalized)) add("夜晚", "慢歌", "soft", "slow", "dream", "night");
  if (/纯音|纯音乐|无人声|instrumental/i.test(normalized)) add("纯音乐", "instrumental", "piano", "ambient", "ost");
  if (/ost|原声|影视|电影|电视剧|动漫|bgm|配乐/i.test(normalized)) add("OST", "soundtrack", "原声", "配乐");
  if (/电子|电音|edm|synth|house|techno/i.test(normalized)) add("电子", "synth", "edm", "house");
  if (/摇滚|rock|punk|alternative/i.test(normalized)) add("摇滚", "rock", "alternative");
  if (/说唱|嘻哈|rap|hip.?hop|trap/i.test(normalized)) add("说唱", "rap", "hip hop", "trap");
  if (/爵士|jazz|blues|bossa/i.test(normalized)) add("爵士", "jazz", "blues", "bossa");
  if (/民谣|folk|country/i.test(normalized)) add("民谣", "folk", "country", "acoustic");
  if (/city\s*pop|citypop|城市流行/i.test(normalized)) add("city pop", "citypop", "昭和");
  if (/lofi|lo-fi|低保真/i.test(normalized)) add("lofi", "lo-fi", "chill", "study");
  if (/没有前奏|没前奏|不要前奏|不带前奏|短前奏|直接开唱/i.test(normalized)) add("短前奏", "直接开唱", "vocal");
  return [...new Set(parts)].join(" ") || extractStyleLabel(raw);
}
...
function extractRequestedArtistName(prompt) {
  ...
  for (const pattern of patterns) {
    ...
    const candidate = cleanQuery(match[1])
      .replace(/^(一下|一些|几首|全部|所有|比较|类似|这种|这个|那个|该|当前)\s*/g, "")
      .replace(/\s*(一下|一些|几首|全部|所有|类似|这种|这个|那个)$/g, "")
      .trim();
...
async function handleRequestedArtistPlayback(prompt, playlist, memory) {
  ...
  if (matches.length) {
    return {
      reply: queuedIndexes.length
        ...
        : `你的歌单里找到 ${matches.length} 首 ${artist}，你可以从下面点播。`,
  ...
  if (netease.length) {
    return {
      reply: `你的歌单里没找到 ${artist}，但网易云搜到 ${netease.length} 个候选；可以点卡片播放，或去 SongID 里播放全部。`,
      recommendations: neteaseRecommendations(netease),
      queued: false,
      queuePreview: [],
      memory
...
function wantsMusicSearch(prompt) {
  ...
  return /我要听|我想听|想听|播放|放一首|给我放|给我播|推荐|找|搜|搜索|检索|查询|查一下|查找|有哪些|有什么|来点|来首|换成|切到|接下来听|下一首听|歌曲列表|歌单列表|曲库列表|随机播放|没有前奏|没前奏|不要前奏|直接开唱/i.test(normalized);
}
...
function isPlainQuestion(prompt) {
  ...
  if (/(我要听|我想听|想听|播放|放一首|给我放|给我播|推荐|找|搜|搜索|检索|查询|查一下|查找|有哪些|有什么|来点|来首|换成|切到|接下来听|下一首听)/i.test(normalized)) return false;
...
function wantsSimilarStyleQueue(prompt) {
  const normalized = normalizeText(prompt);
  return /(类似|相似|像这首|这种|这类|同类|同样|接近).{0,12}(歌|歌曲|音乐|曲子|风格|感觉|氛围|慢|安静|舒缓|纯音|纯音乐)|(?:多|再|继续|接下来).{0,8}(播放|放|排|来|听).{0,16}(类似|相似|这种|这类|慢节奏|慢|安静|舒缓|纯音|纯音乐)|为我多播放/.test(normalized);
}
...
function findSimilarStyleMatches(playlist, prompt, currentTrack, limit = 12) {
  ...
  return playlist.tracks.map((track, index) => {
    ...
    let score = 0;
    if (wantsSlow && /slow|soft|dream|night|moon|blue|ambient|piano|acoustic|lofi|ballad|chill|refrain|雨|夜|梦|月|慢|安静|温柔|舒缓|钢琴|纯音|配乐|原声/.test(raw)) score += 42;
...
function extractDirectTitleQuery(prompt) {
  ...
  text = text
    ...
    .replace(/^(播放|放一下|放一首|放|播一下|播|来一首|我要听|我想听|想听)\s*/i, "")
...
function looksLikeDirectTitlePlayback(prompt) {
  ...
  if (!/^(请|帮我|给我|直接|现在|马上|立刻)?\s*(播放|放一下|放一首|放|播一下|播|来一首|我要听|我想听|想听)\b/i.test(String(prompt || "").trim())) return false;
...
async function handleDsMusicIntent(intent, { prompt, playlist, payload, memory, taste, weather }) {
  ...
  const currentArtistReference = /(该歌手|这个歌手|这位歌手|当前歌手|这个乐队|该乐队|他们|他|她).{0,12}(其他|其它|别的|更多|歌|歌曲|作品)|(?:播放|放|播|来点|多放|多播放|推荐).{0,12}(该歌手|这个歌手|这位歌手|当前歌手|这个乐队|该乐队|他们|他|她)/.test(normalizeText(prompt));
    ...
    return {
      reply: matches.length
        ? queuedIndexes.length
          ? `我只保留了歌词时间戳显示 16 秒内进人声的歌，已排 ${queuedIndexes.length} 首到当前歌曲后面。`
          : `我只列歌词时间戳显示 16 秒内进人声的歌，下面这些更接近“没有前奏”。`
        : "我按歌词时间戳查了一轮，没有找到足够可靠的“16 秒内进人声”歌曲，所以这次不硬排。",
  ...
  if (mode === "recommend_similar" || mode === "recommend_style") {
    ...
    if (!matches.length) {
      ...
      return {
        reply: netease.length
          ? shouldQueue
            ? `当前歌单里没筛到特别稳的 ${neteaseQuery}，我改去网易云搜到了 ${netease.length} 个候选，已放到当前歌曲后面。`
            : `当前歌单里没筛到特别稳的 ${neteaseQuery}，我改去网易云搜到了 ${netease.length} 个候选。`
          : `我按 ${neteaseQuery} 搜了当前歌单和网易云，都没拿到稳的候选。`,
        recommendations: neteaseRecommendations(netease),
    ...
    return {
      reply: matches.length
        ...
        : "我按你的意思筛了一遍当前歌单，但没找到足够稳的候选，所以先不硬排。",
      recommendations: matches.slice(0, 12).map(recommendationFromMatch),
      queued: queuedIndexes.length > 0,
      queuePreview: queuePreviewFromIndexes(playlist, queuedIndexes),
      memory
  ...
  try {
    const reply = await withTimeout(deepSeekIntentJson(
      ...
      [
        ...
        "如果用户说的是风格、类型、语言、氛围，例如“r&b歌曲”“emo歌”“华语歌”“日语歌”“夜晚慢歌”“没有前奏的歌”，intent 必须用 recommend_style，style 填风格词；绝对不要把“r&b歌曲”当成歌名 title。",
        "“没有前奏/没前奏/直接开唱/短前奏”是特殊风格，style 输出“短前奏 直接开唱 vocal fast”，不要输出“伴奏/off vocal/instrumental”。",
        "style 不能只复读用户原词，要扩展成检索语义包。比如“苦情歌”不要输出“情歌”，要输出“失恋 伤感 emo 慢歌 爱而不得 ballad”；“甜甜的情歌”输出“爱情 浪漫 甜歌 love ballad”；“英文R&B”输出“English r&b soul neo soul”。",
        "例如“我想听r&b歌曲，接下来为我播放”=> {\"intent\":\"recommend_style\",\"style\":\"r&b\",\"autoplay\":true,\"confidence\":0.98}。",
        "English requests follow the same rule: “Play some English R&B after this song” => {\"intent\":\"recommend_style\",\"style\":\"English r&b\",\"autoplay\":true,\"confidence\":0.98}.",
        "如果用户明确说播放/放/播/我要听某首歌，intent 用 play_title，title 填歌名。",
        "如果用户说“该歌手/这个歌手/这位歌手/当前歌手/他们/他/她 的其他歌”，intent 必须用 play_current_artist；绝对不要把“该歌手其他歌”当成 title 或搜索词。",
        "例如当前歌手是 Michael Learns To Rock，用户说“我要听该歌手其他歌”，输出 {\"intent\":\"play_current_artist\",\"autoplay\":true,\"confidence\":0.98}。",
        "如果用户说“类似这首/这种/这类/慢节奏/舒缓/多播放类似”，intent 用 recommend_similar，style 填关键词，autoplay=true。",
        "如果用户问当前这首歌、当前歌手、歌词、专辑、创作背景，intent 用 current_track_question。",
        "如果只是裸歌名但像在延续刚才的点歌，intent 用 search_title。",
        "如果用户明确说某歌手的歌，填 artist；例如“接下来为我播放张宇的歌”=> {\"intent\":\"play_artist\",\"artist\":\"张宇\",\"autoplay\":true,\"confidence\":0.95}。",
        "“某某的歌 / 来点某某 / 想听某某”默认把某某理解为歌手，除非语义明显是在说歌名。",
        "不要把未知缩写强行展开；不确定就保持用户原词并降低 confidence。",
        "不要把当前正在播放的歌当成答案。"
      ].join("\n")
    ), 3000, null);
...
function playbackLogicReply(playlist, payload) {
  ...
  return [
    "现在的播放逻辑是分层的：如果你在 Chat 里明确说“我要听某某歌手/风格/歌曲”，我会把匹配到的歌排到当前歌曲后面，等当前歌自然播完再接上。",
    queueCount
      ? `当前后续队列里还有 ${queueCount} 首，优先级最高，会先播队列。`
      : "当前没有手动队列，所以播完会从你的歌单里自动挑下一首。",
    `自动挑歌不是固定顺序：它会避开最近刚播过的歌，再按你导入的当前歌单、最近听歌偏好、第一次进站拿到的天气氛围做一点加权。天气只影响第一次打开后的整体气质，不会每首都拿出来说。`,
    `当前歌单现在有 ${playlist.tracks.length} 首；当前是 ${payload.track.title} - ${payload.track.artist}。如果你觉得它选得不对，可以直接说“接下来听更轻一点的 R&B”或“这类不要了”，我会把这个偏好记进 taste memory。`
  ].join("\n\n");
}
...
function plainLyricText(lyric = "") {
  return lyric
    ...
    .filter((line) => line && !/^(作词|作曲|编曲|制作人|监制|词|曲|arranger|composer|lyricist)\s*[:：]/i.test(line))
    .slice(0, 14)
    .join("\n");
}
...
async function answerCurrentTrackQuestion(prompt, payload, memory) {
  ...
  const fallback = [
    ...
    lyricText
      ? `从歌词开头看，它不是只靠歌名撑情绪，而是在用这些句子把场景摆出来：${lyricPreview}。如果 AI 没接上，我至少能先按歌词和专辑语境聊，不会再用空泛模板糊过去。`
  ...
  try {
    const reply = await aiChat(
      [{ role: "user", content: [
        ...
        `歌手：${track.artist}`,
        `专辑：${track.album || "未知"}`,
        // AI DJ disabled for now. Restore this context line if host copy returns:
      ...
      [
        "你是一个懂音乐、懂影视和流行文化的电台朋友。用户问的是当前正在播放的歌、歌手、专辑、歌词、标题含义、剧集来源或听感。",
        "请正常回答用户的问题，不要复读字段，不要解释你不能做什么，不要把问题改写成命令。",
        // AI DJ disabled for now. If restored, this can include "AI DJ 口播" again.
        "可以结合歌名、歌手、专辑、歌词摘录和公开资料来讲：这首歌大概在表达什么、为什么会放在这里、它和剧集/场景可能有什么关系、听起来有什么特点。",
        "事实不确定时，用自然语言说明不确定，而不是机械拒答；可以给出基于上下文的合理判断，并标明是判断。",
  ...
  try {
    const reply = await aiChat(
      ...
      [
        ...
        "不要把每句话都理解成点歌命令。用户闲聊、追问、抱怨、纠错、问剧情、问歌词、问观点时，就按正常聊天回答。",
        "回答要有具体内容，避免空话、教程口吻和固定句式。不要说“我在”“你可以继续问我”“不用改成命令”。",
        "如果用户在纠正你，先承认你刚才的理解偏差，再根据上下文继续推理；只有信息真的不够时，才问一个具体问题。",
        "不要主动改播放队列；播放和检索已经由外层工具处理。你这里只负责把话答好。",
        "可以结合当前歌曲、最近对话、歌词/专辑/歌手常识来聊。事实不确定时自然说明不确定，不要装懂。",
        "用中文，像一个有音乐品味的朋友认真回应。长短随问题而定，不要为了短而水。"
      ].join("\n")
    );
    return reply || fallback;
  } catch (error) {
    ...
    const reply = await openAiChat([
      {
        ...
        content: [
          ...
          "如果用户是在表达想听某种风格但信息不够，你可以自然追问一个具体问题；如果只是闲聊、纠错、抱怨或追问，就正常回答。",
...
function answerNormalChatFallback(prompt, payload, memory) {
  ...
  const peopleGuess = normalized.match(/你觉得(.+?)和(.+?)(像|像吗|像不像|相似|是不是像)/)
    || normalized.match(/(.+?)和(.+?)(像|像吗|像不像|相似)/);
  if (peopleGuess) {
    const left = peopleGuess[1].replace(/^你觉得/, "").trim();
    const right = peopleGuess[2].trim();
    return `只按名字和当前播放上下文看，我不能直接确认他们长得像不像；如果你说的是封面人物，我需要看到图像细节或专辑信息才敢判断。现在这首的上下文是 ${context || "当前曲目"}，你可以把封面截图再发清楚一点，或者告诉我你想比较的是脸型、气质还是某张照片。`;
  }
  if (/识图|看图|图片|封面|照片/.test(normalized)) {
    return "我现在这个站内 Chat 还不能直接识别你发来的图片或专辑封面。你把图里的文字、歌名、专辑名或你想确认的人名发给我，我可以接着帮你判断。";
  }
  if (/(.+)是(.+)吗[？?]?$|是不是|是否/.test(normalized)) {
    return `我不能凭空确认这个事实。结合当前上下文 ${context || "这首歌"}，我可以帮你往专辑、歌手或公开资料方向查；但如果问题是在问封面里的人是谁，还是需要图像或更明确的专辑名。`;
  }
  if (/是什么|什么意思|写什么|讲什么|表达什么/.test(normalized)) {
    return `我先按当前上下文理解：你问的是 ${context || "这首歌"} 里的某个标题、歌词或说法。现在 AI 连接临时没回到我这里，所以我不硬编；你把那句原文或想问的对象贴全一点，我就按它具体拆。`;
  }
  if (/为什么|怎么|区别|什么意思|像不像|你觉得|能不能|可以吗/.test(normalized)) {
    return `这个问题我会当普通聊天接，不会动播放列表。当前上下文是 ${context || "这首歌"}；如果你是在问它的背景、歌词或剧情关联，我需要拿到更完整的一句描述才不会乱猜。`;
  }
  return `我这边 AI 回复临时没接上，只能先按当前上下文 ${context || "这首歌"} 接一句：你这条不是点歌命令，我不会改队列。把你想追问的对象说完整一点，我继续按聊天回答。`;
}
...
async function generateHostLine(track, nextTrack) {
  ...
  const system = [
    ...
    "每一首都要换一个真正的切入点。不要连续使用“先不急”“让旋律铺开”“场景铺开”“情绪落稳”这类固定句式。",
    ...
    "如果歌曲信息不足，就承认只能从听感切入；不要编造电影、奖项、年代和创作故事。",
...
function externalNeteaseTrack(track = {}) {
  ...
  return {
    id: sourceId,
    title: String(track.title || "网易云歌曲").slice(0, 160),
    artist: String(track.artist || "未知歌手").slice(0, 160),
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === "POST" && pathname === "/api/chat") {
    ...
    if (wantsNoAccompaniment(prompt)) {
      ...
      return json(res, {
        reply: matches.length
          ? "我按“全曲无伴奏/清唱/a cappella”在你的歌单里找，只列标题、专辑或标签里有明确线索的歌；这种需求不能靠普通推荐硬猜。"
          : "我按“清唱 / 无伴奏 / a cappella / vocal only”全局搜了当前歌单，没找到足够可靠的标注。这个条件不能靠歌名硬猜，否则很容易推荐错。",
        recommendations: matches.map((item) => ({
          index: item.index,
          title: item.track.title,
          artist: item.track.artist,
    ...
    if (queryStyleFlags(prompt).noIntro) {
      ...
      return json(res, {
        reply: matches.length
          ? "我按“开口快”重新筛了一遍：优先查第一句歌词出现得早的歌，并排除了明显 intro / 纯音乐 / OST。"
          : "这次没筛到足够稳的“开口快”歌曲；当前歌单信息里没有前奏长度字段，我不想硬凑。",
        recommendations: matches.map((item) => ({
          index: item.index,
          title: item.track.title,
          artist: item.track.artist,
    ...
    if (referencesRecentRecommendations(prompt)) {
      ...
      if (matches.length) {
        await rememberRecommendations(memory, matches);
        return json(res, {
          reply: `我按刚才那批推荐里重新找了一下，匹配到 ${matches.length} 首。`,
          recommendations: matches.map((item) => ({
            index: item.index,
            title: item.track.title,
            artist: item.track.artist,
    ...
    if (isSongCountQuestion(prompt)) {
      ...
      return json(res, {
        reply: matches.length
          ...
          : `我按“${query}”查了歌名，你的歌单里暂时没有特别稳的同名结果。`,
        recommendations,
        memory
      });
    }
    if (wantsFuzzyTitleSearch(prompt)) {
      ...
      return json(res, {
        reply: matches.length
          ? `我按你记得的歌名做了模糊检索，找到 ${matches.length} 个可能版本；你记错一点也没关系，我先把相近的都列出来。`
    ...
    const aliasDefinition = normalizeText(prompt).match(/^(?:记住|以后|下次|以后把|下次把)?\s*(.{1,16}?)(?:就是|指的是|当成|按)(.{1,30})$/);
    if (aliasDefinition) {
    ...
    try {
      const generated = await aiChat(
        ...
        [
          `你是 ${taste.stationName} 的电台伙伴。你可以正常聊天，也可以帮用户从你的歌单里找歌。`,
