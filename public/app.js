...
async function loadFavoritePlaylistMenu() {
  ...
  try {
    ...
    const fallbackPlaylists = fixedNeteasePlaylistIds.map((id) => ({
      id,
      name: fixedNeteasePlaylistNames[id] || `Playlist ${id}`,
      cover: "",
      trackCount: 0,
      containsSong: false
    }));
    const playlistMap = new Map();
    [...fallbackPlaylists, ...(data.playlists || [])].forEach((item) => {
      ...
      playlistMap.set(playlistId, {
        ...
        ...item,
        id: playlistId,
        name: item?.name || fixedNeteasePlaylistNames[playlistId] || `Playlist ${playlistId}`
      });
    });
